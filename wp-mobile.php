...
require_once ($curpath.'/wp-includes/template-functions.php');
require_once ($curpath.'/wp-includes/vars.php');
require_once ($curpath.'/wp-includes/functions.php');
require_once ($curpath.'/wp-includes/wp-db.php');
...
$latest = 0;
if (!isset($_REQUEST["p"])) {
	$latest_entry = mysql_query("SELECT ID FROM $tableposts "
	                           ."WHERE post_date < '$now' "
	                           ."AND post_status = 'publish' "
	                           ."ORDER BY post_date DESC "
	                           ."LIMIT 1"
	                           );
	$p = mysql_result($latest_entry, 0);
	$latest = 1;
}
?>
<?php /* start the blog ! */ $blog=1; include ("wp-blog-header.php"); ?>
<?php
$main_blogfilename = $blogfilename;
...
<html>
<head>
<title><?php echo $blogname ?> mobile edition</title>
...
?>
<p><a href="http://www.alexking.org/software/b2/" target="_blank">WordPress Mobile Edition</a> available at alexking.org.</p>
